...
from app.auth.errors import ThereIsAUserError, ValidationError, WrongDataForAuthError
...
class Auth_user:
    ...
    async def register_user_service(user_data: dict) -> int:
        user_in_db = await DaoAuth.get_user_by_email(user_data['email'])
        if user_in_db is not None:
            raise ThereIsAUserError()

        user_data['password'] = Hasher.get_password_hash(user_data['password'])
        try:
            return await DaoAuth.add_user_to_db(first_name=user_data['first_name'], last_name=user_data['last_name'],
